...
export async function genericPUT<T extends { [***REMOVED***:string]: any }>(request: Request, schema: { safeParse: (a0:any) => { success: true; data: T; } | { success: false; error: ZodError; } }, repo: BaseRepository<T>, identityKey: string): Promise<Response> {
    try {
        await setup();
...
export async function genericGET<T extends { [***REMOVED***:string]: any }>(request: Request, repo: BaseRepository<T>) {
    await setup()
