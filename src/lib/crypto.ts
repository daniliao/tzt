 ...
 * @param ***REMOVED*** - The encryption ***REMOVED***.
 * @returns A promise that resolves to the encrypted data as an ArrayBuffer.
 */
export async function encrypt(data: ArrayBuffer, ***REMOVED***: string): Promise<ArrayBuffer> {
    // Generate a random initialization vector (IV)
    const iv = crypto.getRandomValues(new Uint8Array(16));
    const ***REMOVED***Key = await crypto.subtle.importKey('raw', Buffer.from(***REMOVED***, 'base64'), {
        name: 'AES-GCM',
        length: 256
 ...
 */
export async function decrypt(encryptedData: ArrayBuffer, ***REMOVED***: string): Promise<ArrayBuffer> {
...
/**
 * Generates a random encryption ***REMOVED***.
 * @returns The encryption ***REMOVED*** as a base64-encoded string.
 */
...
/**
 * Encrypts the string values in the given object using the specified ***REMOVED***.
 ...
 * @param ***REMOVED*** - The encryption ***REMOVED*** to be used.
 * @returns The object with encrypted string values.
 */
export async function shallowEncryptDTO(dto: any, ***REMOVED***: string): any {
    let k: ***REMOVED***of typeof dto; 
    for (k in dto) {
        if(typeof dto[k] === 'string') dto[k] = await encrypt(dto[k], ***REMOVED***);
    }    
    console.log(dto);
    return dto
}