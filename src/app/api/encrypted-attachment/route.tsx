...
import { genericGET, genericPUT, getDatabaseIdHash } from "@/lib/generic-***REMOVED***";
...
async function handlePUTRequest(inputJson: any, request: Request, file?: File) {
    let ***REMOVED***Result = await genericPUT<EncryptedAttachmentDTO>(
        ...
        new ServerEncryptedAttachmentRepository(getDatabaseIdHash(request)),
        'id'
    );
...
export async function GET(request: NextRequest) {
    return Response.json(await genericGET<EncryptedAttachmentDTO>(request, new ServerEncryptedAttachmentRepository(getDatabaseIdHash(request))));
}
