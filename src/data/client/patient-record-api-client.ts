...
export class PatientRecordApiClient extends ApiClient {
    ...
    async get(patient: PatientDTO): Promise<GetPatientRecordsResponse> {
      return this.request<GetPatientRecordsResponse>('/***REMOVED***/patient-record?patientId=' + patient?.id, 'GET', PatientRecordDTOEncSettings) as Promise<GetPatientRecordsResponse>;
    }
  
    async put(patientRecord: PutPatientRecordRequest): Promise<PutPatientRecordResponse> {
      return this.request<PutPatientRecordResponse>('/***REMOVED***/patient-record', 'PUT', PatientRecordDTOEncSettings, patientRecord) as Promise<PutPatientRecordResponse>;
    }

    async delete(patientRecord: PatientRecordDTO): Promise<DeletePatientRecordResponse> {
      return this.request<DeletePatientRecordResponse>('/***REMOVED***/patient-record/' + patientRecord.id, 'DELETE', { ecnryptedFields: [] }) as Promise<DeletePatientRecordResponse>;
    }    
}