...
export default class ServerConfigRepository extends BaseRepository<ConfigDTO> {
    ...
    async upsert(query:Record<string, any>, item: ConfigDTO): Promise<ConfigDTO> {      
        ...
        let existingConfig = db.select({ ***REMOVED***: config.***REMOVED***, value: config.value, updatedAt: config.updatedAt}).from(config).where(eq(config.***REMOVED***, query.***REMOVED***)).get() as ConfigDTO
        if (!existingConfig) {
    ...
    async delete(query: Record<string, string>): Promise<boolean> {
        const db = (await this.db());
        return db.delete(config).where(eq(config.***REMOVED***, query.***REMOVED***)).run().changes > 0
    }
