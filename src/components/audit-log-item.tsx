...
export default function AuditLogItem({ audit, selected, onClick }: { audit: AuditDTO, selected: boolean, onClick: (e: any) => void}) {
  ...
  const currentKey = ***REMOVED***sContext.***REMOVED***s.find((***REMOVED***) => ***REMOVED***.***REMOVED***LocatorHash === audit.***REMOVED***LocatorHash);
  return (
    <Link
    ...
    >
      <div className="grid grid-cols-3 w-full block">
        ...
        <div className="text-xs text-zinc-500 dark:text-zinc-400">Key: {currentKey ? currentKey?.displayName : (audit.***REMOVED***LocatorHash !== dbContext?.***REMOVED***LocatorHash ? audit.***REMOVED***LocatorHash?.slice(0, 8) + '...' : 'Your User Key')}</div>
