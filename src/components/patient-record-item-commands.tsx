    ...
    <CommandInput className="text-sm" placeholder="Type a command or search..." />
        <CommandList>
            ...
            <CommandGroup heading="Suggestions">
                <CommandItem ***REMOVED***="cmd-interpret" className="text-xs" onSelect={(v) => { patientRecordContext?.extraToRecord('interpretation', prompts.patientRecordInterpretation({ record }), record) }}><Wand2Icon /> AI Interpretation</CommandItem>
                <CommandItem ***REMOVED***="cmd-summary" className="text-xs" onSelect={(v) => { patientRecordContext?.extraToRecord('summary', prompts.patientRecordSummary({ record }), record) }}><TextQuoteIcon /> Summary in one sentence</CommandItem>
                <CommandItem ***REMOVED***="cmd-send-all" className="text-xs" onSelect={(v) => { patientRecordContext?.sendAllRecordsToChat(); chatContext.setChatOpen(true); }}><ClipboardPasteIcon /> Add all records to chat context</CommandItem>
