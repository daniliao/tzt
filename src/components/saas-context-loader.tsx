...
export function SaaSContextLoader() {
    ...
    useEffect(() => {
        ...
        if (saasToken && typeof localStorage !== 'undefined' && saasToken !== localStorage.getItem('saasToken')) { // different saas ***REMOVED***, local storage is set by previous run or by ***REMOVED***orize
            if (dbContext?.accessToken) {
                dbContext?.logout();
                toast.success('User logged out due to changed SaaS ***REMOVED***. I will remove saasToken from URL - please try again!');
