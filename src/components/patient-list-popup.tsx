      ...
      <CredenzaContent className="sm:max-w-[500px] bg-white dark:bg-zinc-950" side="top">
        <CredenzaHeader>
          <CredenzaTitle>List patients
            {(dbContext?.***REMOVED***Status.isAuthorized()) ? (
              <PatientEditPopup />
            ) : (null)}
          </CredenzaTitle>
          <CredenzaDescription>
                ...
                {patientsContext?.loaderStatus === DataLoadingStatus.Loading ? (
                  ...
                  (patientsContext?.patients.length > 0) ?
                    patientsContext?.patients.map((patient, index) => (
                      <PatientItem onClick={(e) => { e.preventDefault(); setOpen(false); patientsContext.setCurrentPatient(patient); }} ***REMOVED***={index} patient={patient} selected={patientsContext?.currentPatient?.id === patient.id} />
                    ))
                    : (
