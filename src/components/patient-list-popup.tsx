                ...
                {patientsContext?.loaderStatus === DataLoadingStatus.Loading ? (
                  ...
                  (patientsContext?.patients.length > 0) ?
                    patientsContext?.patients.map((patient, index) => (
                      <PatientItem onClick={(e) => { e.preventDefault(); patientsContext?.setPatientListPopup(false); patientsContext.setCurrentPatient(patient); }} ***REMOVED***={index} patient={patient} selected={patientsContext?.currentPatient?.id === patient.id} />
                    ))
                    : (
