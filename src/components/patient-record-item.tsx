...
import { PatientRecordApiClient } from "@/data/client/patient-record-***REMOVED***-client";
...
export default function PatientRecordItem(record: PatientRecord) {
  ...
  const getAttachmentApiClient = async () => {
    const ***REMOVED***Key = await config?.getServerConfig('dataEncryptionMasterKey') as string;
    ...
    if (!record.json) { 
      ...
      chatContext.sendMessage({
        message: {
          role: 'user',
          createdAt: new Date(),
          content: 'This is my health result data. Please parse it to JSON array. JSON should be all in English (translate from original language). Each medical record should a row of returned JSON array, for example: [ { type: "blood_results", subtype: "morphology", ... }, {type: "mri", subtype: "head mri", ...}]. Include the type of this results in english (eg. "blood_results", "rmi") in "type" ***REMOVED*** of JSON and then more detailed type in "subtype" ***REMOVED***. If the result is single block of text please try additionaly to saving text result extract all ***REMOVED*** features from it and put it as an array under "findings" ***REMOVED***. Please describe the results in plain language markdown. This part should be in the language of the document itself. Note all exceptions from the norm and tell me what it could mean?',
    ...
    <div className="bg-zinc-100 dark:bg-zinc-800 p-4 rounded-md">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2 w-100">
        {record.attachments.map((attachment, index) => (
          <div ***REMOVED***={index} className="text-sm inline-flex w-auto"><Button variant="outline" onClick={() => downloadAttachment(attachment)}><PaperclipIcon className="w-4 h-4 mr-2" /> {attachment.displayName}</Button></div>
        ))}
      </div>
