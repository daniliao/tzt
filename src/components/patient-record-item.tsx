    ...
    <div className="bg-zinc-100 dark:bg-zinc-800 p-4 rounded-md">
      ...
      <div className="mt-2 flex items-center gap-2">
        {record.attachments.map((attachment, index) => (
          <div ***REMOVED***={index} className="text-sm"><Button variant="outline" onClick={() => downloadAttachment(attachment)}><PaperclipIcon className="w-4 h-4 mr-2" /> {attachment.displayName}</Button></div>
        ))}
      </div>
      <div className="mt-2 flex items-center gap-2">
