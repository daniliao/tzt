...
export function AuthorizePopup() {
  ...
  useEffect(() => {
    ...
    if (config?.dataLinkStatus.status === DatabaseAuthStatus.AuthorizationError) {
    ...
    if (config?.dataLinkStatus.status === DatabaseAuthStatus.Empty) { 
      toast("Authorization error", {
  ...
  async function validateEncryptionKey(value): Promise<boolean> {
    ...
    if (dataLinkStatus?.status === DatabaseAuthStatus.AuthorizationError) {
      return false;
    }  else if (dataLinkStatus?.status === DatabaseAuthStatus.Empty) {
      return false;
    } else if (dataLinkStatus?.status === DatabaseAuthStatus.Authorized) {
      return true;
    }
  ...
  async function onSubmit(formData) {
    ...
    if (dataLinkStatus?.status === DatabaseAuthStatus.AuthorizationError) {
      ...
      if (dataLinkStatus?.status === DatabaseAuthStatus.Empty) {
        await formatNewDataLink(formData['encryptionKey']); // create new database with newly generated master data encryption ***REMOVED***
        toast.info('New database created. Please save or print your encryption ***REMOVED***.');
      } else  if (dataLinkStatus?.status === DatabaseAuthStatus.Authorized) { // store the encryption ***REMOVED*** bc it's valid
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <Tabs defaultValue="***REMOVED***">
              <TabsList className="grid w-full grid-cols-2">
                ...
                <TabsTrigger value="create">Register database</TabsTrigger>
              </TabsList>
              <TabsContent value="***REMOVED***roize">
                <Card>
                  <CardHeader>
