    ...
    return new Promise (async (resolve, reject) => {
        ...
        if (removePIIMode === 'replace' || removePIIMode === 'both') {
            ...
            if (patientContext?.currentPatient) {
                ...
                if (patientJsonData) {
                    Object.***REMOVED***s(patientJsonData).forEach((***REMOVED***) => {
                        const piiToken = patientJsonData[***REMOVED***] as string;
                        if (piiToken && piiToken.length > 3) piiTokens.push(patientJsonData[***REMOVED***]);
                    });
                }
